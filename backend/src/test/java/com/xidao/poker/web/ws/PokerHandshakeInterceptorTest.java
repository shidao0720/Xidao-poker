...
class PokerHandshakeInterceptorTest {
    ...
    void bindsNewPlayerIdentityFromHandshakeOnce() {
        ServerHttpRequest request = request(
                "ws://localhost/ws/poker?roomId=room-1&playerId=alice_1&playerName=%E7%88%B1%E4%B8%BD%E4%B8%9D"
        );
    ...
    void acceptsReconnectOnlyWithEpochAndTokenTogether() {
        Map<String, Object> attributes = new HashMap<>();
        boolean accepted = interceptor.beforeHandshake(
                request("ws://localhost/ws/poker?roomId=room-1&playerId=alice&connectionEpoch=2&resumeToken=abc"),
                mock(ServerHttpResponse.class),
                mock(WebSocketHandler.class),
                attributes
        );
    ...
    void rejectsPartialReconnectCredentialsBeforeUpgrade() {
        ...
        boolean accepted = interceptor.beforeHandshake(
                request("ws://localhost/ws/poker?roomId=room-1&playerId=alice&connectionEpoch=2"),
                response,
                mock(WebSocketHandler.class),
                new HashMap<>()
        );
